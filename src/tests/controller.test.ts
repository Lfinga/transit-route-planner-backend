...
describe('Routes Controller Tests', () => {
    describe('getRouteStops', () => {
        ...
        it('should return stops when they exist for the route', async () => {
            const mockStops = [
                { stop_id: 1, stop_name: 'Stop 1', location: 'Location 1', stop_sequence: 1 },
                { stop_id: 2, stop_name: 'Stop 2', location: 'Location 2', stop_sequence: 2 }
            ];
        ...
        it('should return 404 when no stops are found', async () => {
            ...
            await expect(getRouteStops(mockRequest as Request, mockResponse as Response))
                .rejects
                .toThrow('No stops found for this route or route does not exist');
